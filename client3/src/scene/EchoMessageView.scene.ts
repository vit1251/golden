...
import { changeScene, messageScrollDown, messageScrollUp, type Area, type Message } from "../store/appSlice.ts";
import { store } from "../store/index.ts";
import { fillEnd, searchArea, searchMessage, searchMessagePosition } from "../util.ts";
...
function matchColor(level: number): string {
    if (level === 1) return '#00aaaa';
    if (level === 2) return '#aa00aa';
    if (level === 3) return '#00aa00';
    return '#C0C0C0';
}
...
export const EchoMessageView = (screen: Screen) => {
    ...
    const area: Area = searchArea(areas, areaIndex);
    const message: Message = searchMessage(messages, messageIndex);
    ...
    useEffect(() => {
        useKeyboard({
            ...
            ArrowLeft: () => {
                // TODO - загрузить предыдущее сообщение...
            },
            ArrowRight: () => {
                // TODO - загрузить следующее сообщение...
            }
        });
    }, []);
    ...
    screen.setForegroudColor('#0000C0'); screen.drawLine(0);
    if (area.summary) {
        screen.setForegroudColor('#e7b10f'); screen.writeText(1, 0, ` ${area.summary} `);
    }
    const areaName: string = ` ${area.name} `;
    const posX: number = 80 - areaName.length - 1;
    screen.setForegroudColor('#e7b10f'); screen.writeText(posX, 0, areaName);

    // Первый столбец
    screen.setForegroudColor('#C0C0C0');
    screen.writeText(0, 1, ` №    : ${msgIndex} of ${msgCount}`); // Номер сообзения в базе
    ...
    // Второй столбец
    screen.setForegroudColor('#C0C0C0');
    screen.writeText(40, 1, `Loc`); // Флаги
    ...
    //
    screen.setForegroudColor('#0000C0'); screen.drawLine(5);
    const msgSize: string = makeHuman(content.length);
    screen.setForegroudColor('#e7b10f'); screen.writeText(1, 5, msgSize);
    ...
    for (const [index, paragraphLine] of paragraphLines.entries()) {
        if (index >= contentIndex) {
            ...
            if (isOriginLine(paragraphLine)) {
                screen.setForegroudColor('#404040');
            }
