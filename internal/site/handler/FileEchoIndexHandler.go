...
func (self *FileEchoIndexHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	/* Context handlers */
	handlersBar := self.renderHandlers()
...
func (self *FileEchoIndexHandler) renderRow(area *mapper.FileArea) widgets2.IWidget {
...
func (self *FileEchoIndexHandler) renderMessageCounter(area *mapper.FileArea) widgets2.IWidget {
	counterWidget := widgets2.NewDivWidget()
...
func (self *FileEchoIndexHandler) renderHandlers() widgets2.IWidget {
	...
	handlerLabel := i18n.GetText(mainLanguage, "FileEchoIndexHandler", "handler-button-create")
	actionBar.Add(widgets2.NewMenuAction().
		SetLink(fmt.Sprintf("/file/create")).
		SetIcon("icofont-update").
		SetLabel(handlerLabel))
