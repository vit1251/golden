...
func (self *EchoMsgIndexHandler) renderHandlers(newArea *mapper.Area) widgets2.IWidget {
	...
	composeAddr := urlManager.CreateUrl("/echo/{area_index}/message/compose").
		...
		Build()
	composeTitle := i18n.GetText(mainLanguage, "EchoMsgIndexHandler", "handler-compose-button")
	...
	treeAddr := urlManager.CreateUrl("/echo/{area_index}/tree").
		SetParam("area_index", newArea.GetAreaIndex()).
		Build()
	treeTitle := i18n.GetText(mainLanguage, "EchoMsgIndexHandler", "handler-tree-button")
	actionBar.Add(widgets2.NewMenuAction().
		SetLink(treeAddr).
		SetIcon("icon-tree").
		SetClass("mr-2").
	...
	markAsReadAddr := urlManager.CreateUrl("/echo/{area_index}/mark").
		SetParam("area_index", newArea.GetAreaIndex()).
		Build()
	markAsReadTitle := i18n.GetText(mainLanguage, "EchoMsgIndexHandler", "handler-mark-as-read-button")
	actionBar.Add(widgets2.NewMenuAction().
	...
	settingsAddr := urlManager.CreateUrl("/echo/{area_index}/update").
		SetParam("area_index", newArea.GetAreaIndex()).
		Build()
	settingsTitle := i18n.GetText(mainLanguage, "EchoMsgIndexHandler", "handler-settings-button")
	actionBar.Add(widgets2.NewMenuAction().
...
func (self *EchoMsgIndexHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	/* Context handlers */
	handlersBar := self.renderHandlers(newArea)
...
func (self *EchoMsgIndexHandler) checkSenderInTwit(msg msg.Message, twitNames []mapper.Twit) bool {
...
func (self *EchoMsgIndexHandler) renderRow(area *mapper.Area, m *msg.Message, myName string) widgets2.IWidget {
