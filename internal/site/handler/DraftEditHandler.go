...
type DraftEditHandler struct {
	registry *registry.Container
}

func NewDraftEditHandler(registry *registry.Container) *DraftEditHandler {
	return &DraftEditHandler{
		registry: registry,
	}
}

func (h *DraftEditHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	mainView := h.makeMainView(newDraft)
...
func (h *DraftEditHandler) makeMainView(newDraft *mapper.Draft) widgets2.IWidget {

	urlManager := um.RestoreUrlManager(h.registry)
